...
__dotfiles_source() {
	local module_name="$(basename "${1}" .zsh | cut -d'-' -f2-)"
...
for mod in $(yq '.modules | keys | join(" ")' "${__DOTFILES_STATUS}"); do
	if [[ "${mod}" != "loader" ]]; then
		...
		eval "${additional_eval}" || (
			echo "Failed to eval mod '${mod}' with:"
			printf "cat <<ZSH\n%s\nZSH\n" "${additional_eval}"
		) >>"${DEBUG_DOTFILES}"
		fi
...
if ! cd "${__DOTFILES_ORIG_PWD}"; then
	echo "Failed to return to ${__DOTFILES_ORIG_PWD} after reloading"
	return 1
fi

printf "Dotfiles finished loading in %.2fs\n" $(( $(date +'%s.%N') - loader_start )) >>"${DEBUG_DOTFILES}"
